...
export function useMap() {
  ...
  function markerHTML(building) {
    ...
    const badges = badgeHTML(building.flags || {})
  ...
  function createMarker(building) {
    ...
    function renderDetail(detail) {
      ...
      const f = building.flags || {}
      const chips = `
        <div class="flex flex-wrap gap-1 mb-2 text-[10px]">
          ${f.has_tb ? '<span class="px-2 py-0.5 bg-red-100 text-red-700 rounded">TB</span>' : ''}
          ${f.tb_medication ? '<span class="px-2 py-0.5 bg-red-50 text-red-700 rounded">Obat TB</span>' : ''}
          ${f.has_hypertension ? '<span class="px-2 py-0.5 bg-amber-100 text-amber-700 rounded">Hipertensi</span>' : ''}
          ${f.mental_illness ? '<span class="px-2 py-0.5 bg-purple-100 text-purple-700 rounded">Gg. Jiwa</span>' : ''}
          ${f.has_clean_water ? '<span class="px-2 py-0.5 bg-blue-100 text-blue-700 rounded">Air Bersih</span>' : ''}
          ${f.is_water_protected ? '<span class="px-2 py-0.5 bg-blue-50 text-blue-700 rounded">Air Terlindungi</span>' : ''}
          ${f.has_toilet ? '<span class="px-2 py-0.5 bg-green-100 text-green-700 rounded">Jamban</span>' : ''}
          ${f.is_toilet_sanitary ? '<span class="px-2 py-0.5 bg-green-50 text-green-700 rounded">Jamban Sehat</span>' : ''}
          ${f.pregnant ? '<span class="px-2 py-0.5 bg-pink-100 text-pink-700 rounded">Hamil</span>' : ''}
          ${f.facility_birth ? '<span class="px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded">Persalinan Faskes</span>' : ''}
          ${f.immunization ? '<span class="px-2 py-0.5 bg-sky-100 text-sky-700 rounded">Imunisasi</span>' : ''}
        </div>`
      const navButtons = `
  ...
  function initMap(el, center, zoom) {
    ...
    // Base layers: OSM (default) and Satellite (Esri World Imagery)
    ...
    // Restore last selected base layer from localStorage (default: OSM)
    let preferredBase = 'osm'
    try {
      const saved = localStorage.getItem('map.baseLayer')
      if (saved === 'satellite' || saved === 'osm') preferredBase = saved
    } catch (_) {}

    // Add the chosen base layer to the map
    tileLayer = preferredBase === 'satellite' ? satelliteLayer : osmLayer
    tileLayer.addTo(map)

    // Add a layers control to switch between base maps
    const baseLayers = { 'Peta': osmLayer, 'Satelit': satelliteLayer }
    const layersControl = L.control.layers(baseLayers, null, { position: 'bottomright', collapsed: true })
    ...
    map.on('baselayerchange', (e) => {
      try {
        ...
        const value = name.includes('satelit') ? 'satellite' : 'osm'
        localStorage.setItem('map.baseLayer', value)
      } catch (_) {}
    })
